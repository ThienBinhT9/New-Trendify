...
import { GetFollowingDTO } from "@/application/dtos/follow.dto";
...
export class GetFollowingUseCase {
  ...
  async execute(dto: GetFollowingDTO) {
    const { viewerId, userId, cursor, query, limit = 20, page = 1 } = dto;

    const isSearchMode = query && query.trim().length > 0;
    if (isSearchMode) {
      const result = await this.followRepo.searchFollowing(userId, query.trim(), limit, page);
    ...
    const result = await this.followRepo.findFollowing(userId, limit, cursor);
    if (result.userIds.length === 0) {
